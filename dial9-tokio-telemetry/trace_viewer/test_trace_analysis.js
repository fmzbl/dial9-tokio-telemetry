...
function testCpuResultCounts() {
  if (cpuResult.pollsWithCpuSamples < 0 || cpuResult.pollsWithSchedSamples < 0)
...
function testFlamegraphTree() {
  ...
  if (cpuSamples.length === 0) return pass("No CPU samples to test flamegraph");
...
function testBuildFgData() {
  ...
  pass(`buildFgData: ${data.nodes.length} nodes, ${data.totalSamples} samples`);
}
